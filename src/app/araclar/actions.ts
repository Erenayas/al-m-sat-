...
import { contacts, payments, stockExpenses, stockVehicles } from "@/db/inventory";
import { EXPENSE_CATEGORIES, PAYMENT_METHODS } from "@/db/inventory";
import { eq } from "drizzle-orm";
import { buildCanonicalKey, normalizeVehicle } from "@/domain/normalize";
import { vehicles } from "@/db/schema";
import type { ActionState } from "@/lib/action-state";

/** Boş string'i undefined'a çevirir — form alanları boşken "" gönderiyor */
...
/** Adı verilen cariyi bulur ya da oluşturur. Aynı isim + telefon tekrar yaratılmıyor. */
async function resolveContact(
  ...
  const [row] = await db
    .insert(contacts)
    .values({ name: trimmed, phone: phone ?? null, kind })
 ...
 * Başarısız olması sorun değil — kohort yalnızca piyasa karşılaştırması
 * ekranını açıyor, kâr hesabı buna bağlı değil.
 */
...
export async function createVehicle(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (v.plate) {
    const [dup] = await sql<{ id: number }[]>`
      select id from stock_vehicles where upper(plate) = upper(${v.plate}) limit 1`;
    if (dup) {
      return {
        status: "error",
  ...
  const sellerId = await resolveContact(v.sellerName, v.sellerPhone, "tedarikci");
...
export async function deleteExpense(formData: FormData): Promise<void> {
  ...
  await db.delete(stockExpenses).where(eq(stockExpenses.id, id));
...
export async function recordSale(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  const [vehicle] = await sql<{ purchase_date: string }[]>`
    select purchase_date from stock_vehicles where id = ${s.vehicleId}`;
  if (!vehicle) return { status: "error", message: "Araç bulunamadı." };
  ...
  const buyerId = await resolveContact(s.buyerName, s.buyerPhone, "musteri");
  ...
  await db
    ...
    .where(eq(stockVehicles.id, s.vehicleId));
